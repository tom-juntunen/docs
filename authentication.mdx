...
description: "Learn how to create a scoped API key in the Plangrep app, pass it as a bearer token on every request, and understand key scopes and error responses."
---
...
<Note>
  The Workspace API Keys endpoints (`/api/api-keys`) use cookie/session authentication from the Plangrep app itself. They are **not** accessible with a bearer API key.
</Note>
...
| `reviews:read` | List and get reviews, list messages, and poll executions |
| `reviews:write` | Create reviews, submit text questions, and cancel review executions |

Review permissions are separate from document-processing job permissions. A review citation can link to project source content, but reading that content through an existing source endpoint still requires the scope that endpoint requires.
