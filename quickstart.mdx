...
All fields are optional. You can pass a `webhookUrl` here to receive a callback when the job finishes instead of polling. See [Polling and webhooks](/guides/polling-and-webhooks) for details.
...
Plangrep now validates each upload, analyzes its page families, calculates the billing counts, and begins processing.
...
| `preflighting` | Validating files, analyzing page families, and reserving credits |
