...
<ParamField body="files" type="array" required>
  ...
  <ParamField body="files[].contentType" type="string" required>
    MIME type of the file (for example `application/pdf`, `text/markdown`, or `application/vnd.openxmlformats-officedocument.wordprocessingml.document`).
  </ParamField>
...
<Note>
  Classification is not an upload input. Requests that supply it are rejected. Plangrep analyzes file content during preflight and assigns `plans`, `specifications`, `documents`, or `mixed`.
</Note>

### Supported file types

The declared `fileName` extension and `contentType` must agree. Supported uploads include:
