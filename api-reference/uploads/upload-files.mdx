...
<Note>
  Do not include a classification in `metadata`. Upload overrides are rejected. Plangrep assigns `plans`, `specifications`, `documents`, or `mixed` during preflight.
</Note>
...
<ResponseField name="uploads" type="array">
  ...
  <ResponseField name="uploads[].classification" type="string | null">
    Analyzed file classification. This is `null` until preflight determines `plans`, `specifications`, `documents`, or `mixed`.
  </ResponseField>
