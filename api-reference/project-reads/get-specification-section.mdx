...
<ParamField query="sectionNumber" type="string">
  Exact, case-insensitive match on the section number. The request returns `404` when it does not match the `sectionId` path parameter.
</ParamField>

<ParamField query="pageStart" type="integer">
  Exact match on the first page of the section. The request returns `404` when it does not match.
</ParamField>

<ParamField query="pageEnd" type="integer">
  Exact match on the last page of the section. The request returns `404` when it does not match.
</ParamField>
