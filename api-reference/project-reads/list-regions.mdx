...
description: "GET /api/open/v1/projects/:projectId/regions — paginated list of extracted drawing regions filterable by canonical region metadata and more."
---

Returns a paginated list of extracted drawing regions within a project. A region is a bounding-box-delimited area detected on a sheet—such as a detail, plan view, section cut, or title block—along with any text extracted from that area. Filter by canonical region metadata to focus on specific content patterns.

<Tip>
  **Sheet-scoped regions:** To list regions belonging to a specific sheet, use the sub-resource endpoint `GET /api/open/v1/projects/{projectId}/sheets/{sheetId}/regions`. It accepts the same filter and pagination parameters as this endpoint, with the sheet identified by the `sheetId` path parameter.
</Tip>
...
<ParamField query="sheetNumber" type="string">
  Exact, case-insensitive match on the parent sheet number (for example, `A5.01`).
</ParamField>
...
<ParamField query="regionNumber" type="string">
  Exact, case-insensitive match on the region number. Combine with `sheetNumber` to resolve a reference such as `3/A5.01`.
</ParamField>

<ParamField query="regionKind" type="string">
  Filter by the detected region kind (e.g. `detail`, `keynotes`, `schedule`, `title_block`).
</ParamField>
...
<ResponseField name="regions" type="ProjectRegionEntity[]">
  ...
  <Expandable title="ProjectRegionEntity fields">
    ...
    <ResponseField name="normalizedData" type="object">
      Structured region data when Plangrep has normalized this region. Shape varies by region kind, such as sheet title blocks, schedules, legends, and placed views.
    </ResponseField>
    ...
    <ResponseField name="artifacts" type="ProjectEntityArtifact[]">
      Array of artifact objects containing URLs to derived outputs, such as `region_crop`, `region_ocr`, and `region_text`. Omitted when `compact=true`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
```bash
curl -X GET "https://plangrep.com/api/open/v1/projects/proj_abc123/regions?limit=50&regionKind=detail&classification=plans" \
  ...
  "regions": [
    {
      ...
      "artifacts": [
        { "type": "region_crop", "contentType": "image/png", "url": "https://...", "readWith": "plangrep_show_artifact_image" },
        { "type": "region_text", "contentType": "text/plain", "url": "https://...", "readWith": "plangrep_show_artifact_file" }
      ]
    }
  ]
}
