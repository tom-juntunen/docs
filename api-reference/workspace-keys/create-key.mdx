---
title: "Create a Scoped Open API Key for Your Workspace"
sidebarTitle: "Create Key"
description: "Creates a new API key with selected scopes and an optional expiration date. The plaintext key is returned only once."
---
...
<ResponseField name="plaintextKey" type="string" required>
  The full bearer token to use in API requests. **This value is shown exactly once and is not stored by Plangrep.** Copy it immediately and store it in a secure secrets manager.
</ResponseField>
