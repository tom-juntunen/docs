...
```
POST https://plangrep.com/api/api-keys
```
...
<ParamField body="scopes" type="OpenApiKeyScope[]" required>
  ...
  | `reviews:read` | List reviews, poll executions, and read messages and citations |
  | `reviews:write` | Create reviews, submit questions, and cancel executions |
</ParamField>
...
<ResponseField name="key" type="OpenApiKey" required>
  ...
  <Expandable title="OpenApiKey fields">
    ...
    <ResponseField name="keyPrefix" type="string">
      The short prefix embedded in the plaintext key, used for identification in list views. The full key is only available in the `plaintextKey` field on this response.
    </ResponseField>

    <ResponseField name="keyLast4" type="string | null">
      The last four characters of the plaintext key, used with `keyPrefix` to identify the key without revealing the secret.
    </ResponseField>

    <ResponseField name="scopes" type="OpenApiKeyScope[]">
      The permission scopes granted to this key. Possible values: `jobs:read`, `jobs:write`, `artifacts:read`, `reviews:read`, `reviews:write`.
    </ResponseField>
...
<Tip>
  Follow the principle of least privilege — only grant the scopes your integration actually needs. Review scopes are independent of job scopes. Reading cited source content still requires the permissions for the source endpoint you call.
</Tip>
...
```bash cURL
curl -X POST https://plangrep.com/api/api-keys \
  -H "Cookie: plangrep_session=YOUR_SESSION_COOKIE" \
  -H "Content-Type: application/json" \
  -d '{
    "label": "My Integration Key",
    "scopes": ["jobs:read", "jobs:write", "artifacts:read", "reviews:read", "reviews:write"]
  }'
```
...
```json
{
  ...
  "plaintextKey": "pgp_open_abc123_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx9f3a"
}
```
