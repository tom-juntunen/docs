...
### Classification during preflight

Do not send a classification with an upload. Plangrep determines it from the file during preflight, and rejects upload requests that try to supply an override.

For PDFs, every page is analyzed as a plan, specification, or document page. A file with one page family becomes `plans`, `specifications`, or `documents`. A file containing more than one page family becomes `mixed`. Non-PDF office and text formats are currently classified as `documents`.

Classification is based on file content and page evidence, not the filename. Upload responses may return `classification: null` until preflight finishes.
...
The response includes the updated job state. The job transitions to `preflighting` while Plangrep validates each file, analyzes its page families, and calculates billing counts. It then moves to `processing`. Poll GET `/api/open/v1/jobs/{jobId}` or use your `webhookUrl` to track progress.
