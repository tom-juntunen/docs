...
curl -X POST https://plangrep.com/api/open/v1/jobs/job_xyz789/files \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -F 'metadata={"files":[{"partName":"file0","fileName":"floor-plan.pdf","contentType":"application/pdf"},{"partName":"file1","fileName":"spec-book.pdf","contentType":"application/pdf"}]}' \
  -F 'file0=@floor-plan.pdf' \
  -F 'file1=@spec-book.pdf'
```

Do not include `classification` in upload metadata. Upload overrides are rejected. During preflight, Plangrep analyzes the file bytes and assigns `plans`, `specifications`, `documents`, or `mixed`. The filename does not determine the result.
...
The job transitions first to `preflighting`, where Plangrep validates each file, analyzes page families, and reserves credits from the resulting page counts. It then transitions to `processing`. See [Polling & Webhooks](/guides/polling-and-webhooks) to track progress from here.
