...
description: "Learn how to track Plangrep job progress by polling the job endpoint or by subscribing to webhook push notifications."
---
...
Plangrep sends an HTTP `POST` request to your URL whenever the job status changes. The request body contains the current job object at each status transition.

The `webhookSigningSecret` returned at job creation time lets you verify that the request originated from Plangrep. Verify the signature on every incoming request before processing it.
...
<Note>
  The webhook payload delivers the current job state at each transition. Treat each delivery as an update snapshot and use the `status` field to drive your application logic.
</Note>
