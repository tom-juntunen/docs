---
title: "Read, Interpret, and Download Plangrep Job Results"
sidebarTitle: "Reading Results"
description: "Learn how to fetch the normalized v1 result JSON, interpret extracted documents, sheets, regions, and evidence records, and download artifacts."
---

Once a Plangrep job completes, you can retrieve the full normalized result JSON and query the processed entities via the project endpoints. This guide shows how to fetch the result, traverse its structure, and download associated artifacts.

## Fetch the result JSON

Call `GET /api/open/v1/jobs/{jobId}/result` after the job reaches a terminal status:

```bash
curl https://plangrep.com/api/open/v1/jobs/job_xyz789/result \
  -H "Authorization: Bearer YOUR_API_KEY"
```

The response is a single normalized JSON object:

```json
{
  "schemaVersion": "v1",
  "job": {
    "id": "job_xyz789",
    "projectId": "proj_abc123",
    "status": "completed",
    "resultUrl": "/api/open/v1/jobs/job_xyz789/result",
    "createdAt": "2024-01-15T10:00:00Z",
    "completedAt": "2024-01-15T10:05:00Z",
    "expiresAt": "2024-02-14T10:00:00Z",
    "externalId": null,
    "error": null
  },
  "usage": {
    "pageCount": 26,
    "successfulPageCount": 26,
    "drawingPageCount": 26,
    "documentPageCount": 0,
    "successfulDrawingPageCount": 26,
    "successfulDocumentPageCount": 0,
    "creditsDebited": 260
  },
  "documents": [
    {
      "documentId": "project_file_abc123",
      "fileName": "trenton_plans.pdf",
      "classification": "plans",
      "contentType": "application/pdf",
      "fileSize": 8752555,
      "status": "ready",
      "pageCount": 26,
      "drawingPageCount": 26,
      "documentPageCount": 0,
      "successfulPageCount": 26,
      "successfulDrawingPageCount": 26,
      "successfulDocumentPageCount": 0,
      "error": null,
      "sheets": [
        {
          "id": "project_sheet_001",
          "sheetId": "project_sheet_001",
          "sheetNumber": "G-001",
          "sheetName": "TRENTON READINESS CENTER - RENOVATE",
          "pageNumber": 1,
          "status": "ready",
          "artifacts": []
        }
      ],
      "artifacts": []
    }
  ],
  "evidence": [
    {
      "id": "project_sheet_region:project_sheet_region_001",
      "type": "project_sheet_region",
      "sourceRef": "source-catalog://project_sheet_region:project_sheet_region_001",
      "sheetNumber": "G-001",
      "regionKind": "cover_sheet",
      "normalizedData": {
        "sheetNumber": "G-001",
        "sheetTitle": "Trenton Readiness Center - Renovate"
      },
      "sourceCallouts": [],
      "text": null
    }
  ],
  "warnings": [],
  "errors": [],
  "generatedAt": "2024-01-15T10:05:00Z"
}
```

Check `errors` first. A `completed_with_errors` job may have partial results alongside per-document error entries describing what could not be processed.

---

## Iterate over documents and sheets

Each entry in the `documents` array is a `ResultDocument` that maps back to a file you uploaded. Each document contains a `sheets` array with extracted sheet metadata:

```python
for doc in result["documents"]:
    print(f"File: {doc['fileName']} ({doc['classification']})")
    for sheet in doc.get("sheets", []):
        print(
            f"  Sheet {sheet['sheetNumber']}: "
            f"{sheet['sheetName']} (page {sheet['pageNumber']})"
        )
```

Sheet objects include fields such as `id`, `sheetId`, `sheetNumber`, `sheetName`, `pageNumber`, `status`, and `artifacts` for associated renderings.

---

## Use the evidence array

The top-level `evidence` array aggregates all citable source records extracted from every processed file. Use it to build indexes, feed downstream AI pipelines, or power search UIs:

```python
for ev in result["evidence"]:
    label = ev.get("label", "")
    snippet = ev.get("text", "")[:100]
    print(f"[{ev['type']}] {label} — {snippet}")
```

Evidence records carry a `type` field, such as `"project_file"`, `"sheet"`, `"project_sheet_region"`, `"project_specification"`, or `"project_doc"`, and additional type-specific fields. Sheet-region evidence can include `normalizedData` for structured region content; when it does, inline `text` may be omitted or `null`. Use each record's `sourceRef` value as the `sourceCatalogUri` when you call the source-text endpoint for exact source text. For specification evidence, use the specification section endpoints to fetch full section text and structured requirements.

---

## Download artifacts

Artifacts are authenticated URLs pointing to rendered outputs on your organization's result snapshot. Fetch an artifact by passing its `path` as a query parameter:

```bash
curl "https://plangrep.com/api/open/v1/jobs/job_xyz789/artifact?path=PATH_FROM_ARTIFACT_URL" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  --output result.pdf
```

Plangrep generates several artifact types for each document and sheet:

| Type | Description |
|---|---|
| `source_file` | Original uploaded source file |
| `specification_file` | Source or derivative PDF for a specification document |
| `sheet_pdf` | Derived single-sheet PDF |
| `sheet_render` | Full-resolution sheet rendering |
| `doc_markdown` | Converted document Markdown |
| `doc_pdf` | Converted document PDF |
| `thumbnail` | Low-resolution preview image |
| `viewer` | Interactive viewer URL |
| `region_crop` | Cropped image of a specific region |
| `region_text` | Extracted text for a region |
| `region_ocr` | Raw OCR output for a region |
| `model` | Model or structured machine-readable output |
| `artifact` | Generic artifact when a more specific type does not apply |

Every artifact object includes a non-empty `contentType`. File viewers use the parent source file's MIME type, and artifacts with an unknown type use `application/octet-stream`. Artifact objects can include `readWith`, either `plangrep_show_artifact_image` or `plangrep_show_artifact_file`.

---

## Query project entities

After processing completes, you can query individual entity types on the project using paginated list endpoints. These endpoints support filtering and are useful for ongoing workflows that span multiple jobs.

```bash
# List all sheets in a project
curl "https://plangrep.com/api/open/v1/projects/proj_abc123/sheets?limit=50" \
  -H "Authorization: Bearer YOUR_API_KEY"

# List detail regions
curl "https://plangrep.com/api/open/v1/projects/proj_abc123/regions?regionKind=detail&limit=100" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

Paginate through results using the `cursor` parameter returned in each response.

<Tip>
  Use `compact=true` on entity list endpoints to strip artifact URL fields and reduce response size for indexing workflows.
</Tip>
