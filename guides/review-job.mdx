...
description: "Create a review, submit a question, poll its durable execution, and read the cited assistant answer."
---
...
Your bearer key needs `reviews:write` to create reviews, submit questions, and cancel executions. It needs `reviews:read` to list and get reviews, poll executions, and read messages.
...
Poll while status is `queued` or `running`. Stop when it reaches `completed`, `failed`, or `cancelled`. The execution reports token usage after settlement.
...
Find the assistant message whose ID matches `execution.assistantMessageId`. Each citation includes a `label` and `sourceCatalogUri`; use source search, source text, and project entity endpoints to read the cited source. Those existing source endpoints keep their own permission requirements.
...
Reviews currently supports text questions, polling, a server-selected model, the existing review-token credit policy, and sources already ingested and indexed in the project.

It does not support browser or WebSocket streaming, voice, attachments, model selection, auto-runs, coverage graphs, exports, or document-job webhooks. Review responses only return `/api/open/v1/...` URLs. They never expose runtime session URLs, cookies, capability tokens, stream endpoints, orchestrator paths, or internal container job IDs.
