  ...
  "hits": [
    {
      ...
      "sourceCatalogUri": "plangrep://projects/proj_abc123/specifications/spec_001/sections/section_07210",
      "source": {
        "type": "specification",
        "specId": "spec_001",
        "sectionId": "section_07210",
...
Each hit includes a relevance `score`, a `snippet` of matching text, and source metadata. For specification hits, use `specId` and `sectionId` with [Get Spec Section](/api-reference/project-reads/get-specification-section) to retrieve full section text and structured evidence. For non-specification hits, you can use `sourceCatalogUri` with [Reading bounded source text](#reading-bounded-source-text).
...
For specification sources, call `GET /api/open/v1/projects/{projectId}/specifications/{specId}/sections/{sectionId}` instead. That endpoint returns untruncated section `text`, `textLength`, parts, products, requirements, warnings, and facets.
